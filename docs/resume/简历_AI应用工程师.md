...
- **设计可扩展的工具编排框架**：定义 `ToolDescriptor` 元数据层（category/cost_hint/requires/async_capable/produces_artifacts），实现工具"注册即生效、启动时依赖自检"，文献检索/RAG 问答/脚本生成/headless 仿真/科学计算/知识读写等 15 个工具统一经 OpenAI Function Calling 接入，新增工具零编排改动（目录自动发现为 M2 规划，当前为手动注册列表）。
...
- **建立分层测试与鲁棒性保障**：157 非冒烟回归测试（162 collected，5 冒烟跳过）覆盖工具契约、故障路径（RAG 宕机明确报错不幻觉、TMS 超时返回结构化 partial）与跨会话记忆召回；真实冒烟分级（headless 体素化 25s / GUI 求解 420s），B2 路径行为零回归。
...
科研智能体：ReAct 循环驱动 15 工具（RAG/仿真/优化/检索/记忆），工具调用成功率实测 100%。

- 设计 ToolDescriptor 元数据 + 可扩展工具框架，15 工具经 Function Calling 统一编排，启动时依赖自检。
